...
void RadixHTLocalSourceState::ExecuteTask(RadixHTGlobalSinkState &sink, RadixHTGlobalSourceState &gstate,
	...
	case RadixHTSourceTaskType::FINALIZE: {
		...
		if (ht->trace_lineage) {
			reinterpret_cast<HALog*>(chunk.log_per_thread.get())->flushmove_log = move(reinterpret_cast<HALog*>(ht->log_per_thread.get())->flushmove_log);
		}
		break;
...
void RadixHTLocalSourceState::Finalize(RadixHTGlobalSinkState &sink, RadixHTGlobalSourceState &gstate) {
	...
	ht->trace_lineage = ClientConfig::GetConfig(sink.context).trace_lineage;
	if (ht->trace_lineage) {
		ht->log_per_thread = make_shared<HALog>(0);
	}
...
void RadixHTLocalSourceState::Scan(RadixHTGlobalSinkState &sink, RadixHTGlobalSourceState &gstate, DataChunk &chunk) {
	...
	if (chunk.trace_lineage) {
		auto ptrs = FlatVector::GetData<data_ptr_t>( scan_state.chunk_state.row_locations);
		unique_ptr<data_ptr_t[]> addresses_copy(new data_ptr_t[scan_chunk.size()]);
		std::copy(ptrs, ptrs + scan_chunk.size() , addresses_copy.get());
		auto lop = reinterpret_cast<HALog*>(chunk.log_per_thread.get());
		lop->scan_log.push_back({move(addresses_copy), scan_chunk.size()});
	}
#endif
