...
#ifdef LINEAGE
		if (trace_lineage) {
		    auto ptrs = FlatVector::GetData<data_ptr_t>( fm_state.scan_state.chunk_state.row_locations);
		    unique_ptr<data_ptr_t[]> src_addresses_copy(new data_ptr_t[fm_state.groups.size()]);
		    std::copy(ptrs, ptrs + fm_state.groups.size(), src_addresses_copy.get());
