...
template<class T>
vector<idx_t> local_factorize(shared_ptr<OperatorLineage> lop, idx_t col_idx) {
	...
	lop->n_interventions = dict.size();
	return codes;
}

vector<idx_t> factorize(PhysicalOperator* op, shared_ptr<OperatorLineage> lop, std::unordered_map<std::string, std::vector<std::string>> columns_spec) {
	...
	for (idx_t i=0; i < scan->names.size(); i++) {
		if (scan->names[i] == col_name) {
			...
			if (scan->types[i] == LogicalType::INTEGER) {
				col_codes = local_factorize<int>(lop, i);
			} else if (scan->types[i] == LogicalType::VARCHAR) {
				col_codes = local_factorize<string>(lop, i);
			} else if (scan->types[i] == LogicalType::FLOAT) {
				col_codes = local_factorize<float>(lop, i);
			}
...
void TableIntervene(shared_ptr<OperatorLineage> lop, PhysicalOperator* op, std::unordered_map<std::string, std::vector<std::string>> columns_spec) {
	if (lop->annotations.empty()) return;
	bool cache_on = false;
	DataChunk result;
	...
	do {
		...
		for (idx_t i=0; i < result.size(); ++i) {
			...
			lop->annotations[oid] = lop->annotations[iid];
		}
	} while (cache_on || result.size() > 0);
}

void  FilterIntervene(shared_ptr<OperatorLineage> lop, PhysicalOperator* op) {
  lop->n_interventions = op->children[0]->lineage_op->n_interventions;
  if ( lop->n_interventions == 1) {
		return;
  }
  ...
  idx_t current_thread = 0;
  idx_t log_id = 0;
  vector<idx_t> child_annotations = op->children[0]->lineage_op->annotations;
  ...
  do {
		...
		for (idx_t i=0; i < result.size(); ++i) {
			idx_t iid = in_index[i];
			lop->annotations.push_back(child_annotations[iid]);
		}
  } while (cache_on || result.size() > 0);
}

void  JoinIntervene(shared_ptr<OperatorLineage> lop, PhysicalOperator* op) {
  idx_t left_n_interventions =  op->children[0]->lineage_op->n_interventions;
  idx_t right_n_interventions =  op->children[1]->lineage_op->n_interventions;
  lop->n_interventions = left_n_interventions * right_n_interventions;
  if ( lop->n_interventions == 1) {
		return;
  }
  ...
  vector<idx_t> lhs_child_annotations = op->children[0]->lineage_op->annotations;
  vector<idx_t> rhs_child_annotations = op->children[1]->lineage_op->annotations;
  ...
  if (left_n_interventions > 1 && right_n_interventions > 1) {
		do {
			...
			for (idx_t i=0; i < result.size(); ++i) {
				...
				lop->annotations.push_back(lhs_child_annotations[lhs] * right_n_interventions + rhs_child_annotations[rhs]);
			}
		} while (cache_on || result.size() > 0);
		...
		do {
			...
			for (idx_t i=0; i < result.size(); ++i) {
				idx_t lhs = lhs_index[i];
				lop->annotations.push_back(lhs_child_annotations[lhs]);
			}
		} while (cache_on || result.size() > 0);
		...
		do {
			...
			for (idx_t i=0; i < result.size(); ++i) {
				idx_t rhs = rhs_index[i];
				lop->annotations.push_back(rhs_child_annotations[rhs]);
			}
		} while (cache_on || result.size() > 0);
  }
...
template<class T>
vector<vector<T>> SumRecompute(PhysicalOperator* op, shared_ptr<OperatorLineage> lop,
...
template<class T>
vector<vector<T>> CountRecompute(PhysicalOperator* op, shared_ptr<OperatorLineage> lop,
                               BoundAggregateExpression& aggr, idx_t n_interventions,
                               idx_t n_groups, vector<idx_t> aggregate_input_idx) {
  DataChunk result;
  idx_t global_count = 0;
...
void  HashAggregateIntervene(shared_ptr<OperatorLineage> lop, PhysicalOperator* op) {
  ...
  for (idx_t i=0; i < aggregates.size(); i++) {
	  ...
	  if (name == "sum" || name == "sum_no_overflow") {
		  ...
		  new_vals = SumRecompute<int>(op, lop, aggr, lop->n_interventions, n_groups, aggregate_input_idx);
	  } else if (name == "count" || name == "count_star") {
		  // use case statement to choose the right Recompute function. Specialize it for data type and aggregate function
		  new_vals = CountRecompute<int>(op, lop, aggr, lop->n_interventions, n_groups, aggregate_input_idx);
	  } else if (name == "avg") {
		  vector<vector<int>> new_vals_count = CountRecompute<int>(op, lop, aggr, lop->n_interventions, n_groups, aggregate_input_idx);
		  vector<vector<int>> new_vals_sum = SumRecompute<int>(op, lop, aggr, lop->n_interventions, n_groups, aggregate_input_idx);
...
void  UngroupedAggregateIntervene(shared_ptr<OperatorLineage> lop, PhysicalOperator* op) {
...
void GenIntervention(PhysicalOperator* op, std::unordered_map<std::string, std::vector<std::string>> columns_spec) {
  for (idx_t i = 0; i < op->children.size(); i++) {
	  GenIntervention(op->children[i].get(), columns_spec);
  }
  ...
  if (op->type == PhysicalOperatorType::TABLE_SCAN) {
	  ...
	  op->lineage_op->annotations = factorize(op, op->lineage_op, columns_spec);
  }
}
...
void GenRandomIntervention(PhysicalOperator* op,
                           ...
                           idx_t distinct) {
  for (idx_t i = 0; i < op->children.size(); i++) {
	  GenRandomIntervention(op->children[i].get(), columns_spec, distinct);
  }
  ...
  if (op->type == PhysicalOperatorType::TABLE_SCAN) {
	  ...
	  op->lineage_op->annotations = random_unique(op->lineage_op, distinct);
  }
}

void Intervention(PhysicalOperator* op, std::unordered_map<std::string, std::vector<std::string>> columns_spec) {
  for (idx_t i = 0; i < op->children.size(); i++) {
	  Intervention(op->children[i].get(), columns_spec);
  }
 ...
 // } else
  if (op->type == PhysicalOperatorType::FILTER) {
	  FilterIntervene(op->lineage_op, op);
	  ...
	  JoinIntervene(op->lineage_op, op);
  } else if (op->type == PhysicalOperatorType::HASH_GROUP_BY) {
	  HashAggregateIntervene(op->lineage_op, op);
  } else if (op->type == PhysicalOperatorType::UNGROUPED_AGGREGATE) {
	  UngroupedAggregateIntervene(op->lineage_op, op);
  } else if (op->type == PhysicalOperatorType::PROJECTION) {
	  op->lineage_op->annotations = op->children[0]->lineage_op->annotations;
	  op->lineage_op->n_interventions = op->children[0]->lineage_op->n_interventions;
  }
}
...
void LineageManager::Why(int qid, int k, string columns_spec_str, int distinct=0) {
  ...
  // 2. Prune: 	needed only when intervention eval would generate heavy computations in JOIN/FILTER
  // 			or if it'd help to use less memory space.
  // Prune(op, {});

  // 3. Prepare base interventions; should be one time cost per DB
  if (distinct <= 0) {
	  GenIntervention(op, columns_spec);
  } else {
	  GenRandomIntervention(op, columns_spec, distinct);
  }
