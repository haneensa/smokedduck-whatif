...
if plot_scale:
    ...
    #dense_data_v2 = get_data(f"fade_data/dense_all_0.1_april19.csv", 1000)
    dense_data_v2 = get_data(f"fade_data/batch_dense_0.1_nofilterOnprune.csv", 1000)
...
if best:
    ...
    p += geom_line()
    p += axis_labels('Batch Size', "Latency (ms, log)", "log10", "log10",
    ...
    if plot:
        ...
        p = ggplot(data_distinct_2048, aes(x='query',  y="speedup", color=cat, fill=cat, group=cat))
        p += geom_bar(stat=esc('identity'), alpha=0.8, position=position_dodge(width=0.9), width=0.88)
        p += axis_labels('Query', "Speedup (log)", "discrete", "log10")
        p += legend_bottom
        p += legend_side
        p += facet_grid(".~sf_label~prune_label~n", scales=esc("free_y"))
        ggsave(f"figures/{prefix}_fade_2048_sf1_speedup.png", p, postfix=postfix, width=8, height=3, scale=0.8)
        ...
        p += legend_bottom
        p += facet_grid(".~sf_label", scales=esc("free_y"))
        ggsave(f"figures/{prefix}_fade_2048_sf1_opt_speedup.png", p, postfix=postfix, width=4, height=2.5, scale=0.8)
...
if vec:
    ...
    if plot:
        ...
        ggsave(f"figures/{prefix}_fade_vec.png", p, postfix=postfix, width=4, height=3, scale=0.8)
...
if workers:
    ...
    if plot:
        ...
        ggsave(f"figures/{prefix}_fade_threading_latency_sf1_line.png", p, postfix=postfix, width=10, height=4, scale=0.8)
        ...
        ggsave(f"figures/{prefix}_fade_threading_speedup_sf1_line.png", p,postfix=postfix, width=10, height=4, scale=0.8)
        ...
        p += legend_side
        p += facet_grid(".~sf_label~n", scales=esc("free_y"))
        ggsave(f"figures/{prefix}_fade_threading_speedup_2048_sf1_line.png", p, postfix=postfix,width=4, height=2.5, scale=0.8)
...
if print_summary:
    ...
    if pruning:
        ...
        print(con.execute(""" select sf, qid,
            avg(speedup), min(speedup), max(speedup)
            from prune_data
            group by sf, qid
            order by sf, qid""").df())
