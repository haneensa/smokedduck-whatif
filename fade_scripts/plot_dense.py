...
if True:
    ...
    if include_dbt:
        ...
        select sf, query, dbt.prune dbt_prune, fade.prune, fade.num_threads, dbt.prob,
        ...
        #p += legend_side
        p += legend_bottom
        p += facet_grid(".~prune", scales=esc("free_x"), space=esc("free_x"))
        ggsave(f"figures/fade_dbt_vs_fade.png", p, width=4, height=3, scale=0.8)
