...
fig1_data = con.execute("""select 'Fade' as system, query, prune, sf, eval_time_ms from data where n=1 and num_threads=1
    UNION ALL select 'Original' as system, query, prune, sf, query_timing*1000 as eval_time from data where n=1 and num_threads=1
    UNION ALL select 'LQ' as system, query, prune, sf, lineage_capture_time*1000 as eval_time from data where n=1 and num_threads=1
        """).df()
cat = "system"
