...
print(con.execute("""select system, card, g, n, is_scalar, alpha, 
    ...
    """).df())
scale_data = con.execute("""select t2.system, card, g, n, t2.is_scalar, alpha, t2.num_threads,
    ...
    from (select * from micro where num_threads=1 and is_scalar='False' and system='backward') t1 JOIN
    (select * from micro) t2
    USING (card, g, alpha, n)
    """).df()
...
p += geom_line(stat=esc('identity'))
p += axis_labels('#threads', "runtime (log, ms)", 'continuous', 'log10')
p += legend_side
p += facet_grid(".~alabel~glabel", scales=esc("free_y"))
