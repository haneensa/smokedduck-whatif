...
start = time.time()
# update q1
q1_exp = f"""select l_linestatus as e1, l_partkey as e2,l_quantity as x,
sum(2.0*l_extendedprice - l_extendedprice) as sy,
count(l_extendedprice - l_extendedprice) as cy
from lineitem group by l_quantity, l_linestatus, l_partkey"""
q1_exp_out_diff = con.execute(q1_exp).df()

q1_exp_out = con.execute(f"""select diff.e1, diff.e2, x, base.sy-diff.sy as sy, base.cy-diff.cy as cy
from q1_exp_out_diff as diff JOIN q1_out as base using (x)""").df()


q = f"""
select e1, e2, ( (count(r.x)*sum(r.x*r.sy/r.cy)) - (sum(r.x)*sum(r.sy/r.cy)) ) / ((count(r.x)*(sum(r.x*r.x)) - (sum(r.x))*(sum(r.x)))) as slope
